...
class VoiceCommandReceiver : BroadcastReceiver() {
    ...
    private fun isAuthorizedCaller(context: Context, intent: Intent): Boolean {
        ...
        Log.d(TAG, "Caller package: $callingPackage")
    ...
    private fun handleCreateTask(context: Context, uri: Uri) {
        ...
        scope.launch(Dispatchers.IO) {
            try {
                // Generate task with subtasks using deterministic template
                ...
                val savedTask = taskRepository.getTaskById(taskId).first()
                
                withContext(Dispatchers.Main) {
                    // Provide voice feedback
                    val subtaskCount = savedTask?.subtasks?.size ?: 0
                ...
                withContext(Dispatchers.Main) {
                    val feedback = when {
                        ...
                        incompleteTasks.size == 1 -> {
                            ...
                            val remaining = task.subtasks.count { !it.isCompleted }
    ...
    private fun handleCompleteTask(context: Context, uri: Uri) {
        ...
        scope.launch(Dispatchers.IO) {
            try {
                ...
                var matchedSubtask: Pair<Task, Subtask>? = null
                ...
                withContext(Dispatchers.Main) {
                    when {
                        matchedSubtask != null -> {
                            ...
                            val remaining = task.subtasks.count { !it.isCompleted && it.id != subtask.id }
                            val feedback = if (remaining > 0) {
                                "Completed ${subtask.title}. $remaining steps remaining."
                            } else {
                                "Completed ${subtask.title}. Task finished!"
                            }
                            ttsManager.speak(feedback)
                        }
                        matchedTask != null -> {
                            ...
                            scope.launch(Dispatchers.IO) {
                                taskRepository.updateTaskCompletion(matchedTask.id, true)
                            }
